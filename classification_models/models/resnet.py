...
from ._common_blocks import ChannelSE, AugmentedConv2d
...
def residual_augmented_conv_block(filters, stage, block, strides=(1, 1), attention=None, cut='pre'):
    ...
    def layer(input_tensor):
        ...
        # x = layers.ZeroPadding2D(padding=(1, 1))(x)
        x = AugmentedConv2d(filters, (3,3))(x)
        # x = layers.Conv2D(filters, (3, 3), name=conv_name + '2', **conv_params)(x)
...
def residual_augmented_bottleneck_block(filters, stage, block, strides=None, attention=None, cut='pre'):
    """The identity block is the block that has no conv layer at shortcut.
    # Arguments
        input_tensor: input tensor
        kernel_size: default 3, the kernel size of
