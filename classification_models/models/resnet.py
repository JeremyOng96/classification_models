...
def augmented_residual_conv_block_2(filters, stage, block, strides=(1, 1), attention=None, cut='pre'):
    """The identity block is the block that has no conv layer at shortcut.
    # Arguments
        input_tensor: input tensor
        kernel_size: default 3, the kernel size of
    ...
    def layer(input_tensor):
        ...
        if str(stage+1) in "234":
            x = AugmentedConv2D(filters, (3,3), stage=f"{stage+1}_b",block=f"{block+1}_b")(x)
        else:
            x = layers.ZeroPadding2D(padding=(1, 1))(x)
            x = layers.Conv2D(filters, (3, 3), name=conv_name + '2', **conv_params)(x)
...
MODELS_PARAMS = {
    ...
    'resnet18aa2':ModelParams('resnet18aa2', (2,2,2,2), augmented_residual_conv_block_2,None),
    'resnet34aa2':ModelParams('resnet34aa2',(3,4,6,3), augmented_residual_conv_block_2,None),
}
...
def ResNet18AA2(input_shape=None, input_tensor=None, weights=None, classes=1000, include_top=True, **kwargs):
    return ResNet(
        MODELS_PARAMS['resnet18aa2'],
        input_shape=input_shape,
        input_tensor=input_tensor,
        include_top=include_top,
        classes=classes,
...
def ResNet34AA2(input_shape=None, input_tensor=None, weights=None, classes=1000, include_top=True, **kwargs):
    return ResNet(
        MODELS_PARAMS['resnet34aa2'],
        input_shape=input_shape,
        input_tensor=input_tensor,
        include_top=include_top,
        classes=classes,
